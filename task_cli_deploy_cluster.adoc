...
Das Deploy Administration Utility enthält eine einzige Version von ONTAP Select. Wenn Sie Cluster mit einer früheren Version von ONTAP Select implementieren möchten, müssen Sie zuerst das ONTAP Select-Image der Deploy-Instanz hinzufügen. Weitere Informationen finden Sie unter link:task_cli_deploy_image_add.html["Fügen Sie ein ONTAP Select-Image zur Bereitstellung hinzu"] .
...
** Registrieren Sie einen von vCenter gemanagten Host:
...
. Wählen Sie den Node aus und führen Sie die Basiskonfiguration durch:
