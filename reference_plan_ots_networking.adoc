...
NOTE: Weitere Informationen zum Konfigurieren eines vSwitch auf einem Hypervisor-Host und zur Funktion der Hochgeschwindigkeitsschnittstelle finden Sie im link:concept_nw_concepts_chars.html["Deep-Dive-Networking"] Abschnitt.
...
Weitere Informationen zu Bereitstellungen mit vier NIC finden Sie im link:concept_nw_concepts_chars.html["Deep-Dive-Networking"] Abschnitt.
...
* VSphere (nur ESXi) entweder ein vSphere-Server oder ein ESXi-Host, je nachdem, wie der Host in Ihrer Cluster-Bereitstellung gemanagt wird
