...
Nein. Die ONTAP Select VM verwendet unabhängig-persistente Laufwerke, die von VMware-basierten Snapshots ausgeschlossen sind. Die einzige unterstützte Methode zum Backup von ONTAP Select ist SnapMirror oder SnapVault.
...
* ONTAP Select Deploy erkennt, ob Lizenzen und Supportverträge durch den Kauf einer aktualisierten Lizenzdatei von NetApp verlängert werden. Die Lizenzdatei (.NLF) enthält das Kapazitäts-, Start- und Enddatum und wird auf dem generiert link:https://mysupport.netapp.com/site/["NetApp Support Website"^]und dann auf dem Server „Bereitstellen“ aktualisiert.
+
